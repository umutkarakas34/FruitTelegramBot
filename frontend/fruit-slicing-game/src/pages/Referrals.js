...
const Referrals = () => {
  ...
  const [refEarningClaimDate, setrefEarningClaimDate] = useState(0);
  ...
  const getPoints = async () => {
    try {
      ...
      const response = await api.get('/user/get-referral-tokens', { telegramId: decryptedTelegramId });

      setRefEarning(response.data.ref_earning);
      setrefEarningClaimDate(response.data.ref_earning_claim_date)
    } catch (error) {
